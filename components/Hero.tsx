...
import { ArrowRight, BadgePercent, Bike, Clock, MapPin, Sparkles, Tags } from "lucide-react";
import { createWhatsAppLink } from "@/lib/whatsapp";
import { storeConfig } from "@/config/store";
import { WhatsAppIcon } from "@/components/WhatsAppIcon";

export function Hero() {
  const whatsappLink = createWhatsAppLink("Bonjour ASYA MARKET, je souhaite commander mes courses.");
        ...
        <div className="grid gap-7 p-5 sm:p-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-stretch lg:p-10 xl:p-12">
          <div className="flex flex-col justify-center lg:pr-4">
            ...
            <p className="mt-3 text-lg font-bold leading-7 text-sun sm:text-xl">
              Votre marché de proximité pour vos courses du quotidien
            </p>
            <p className="mt-4 max-w-2xl text-sm font-medium leading-7 text-zinc-200 sm:text-base">
              Découvrez nos produits, promotions et nouveautés. Commandez facilement via WhatsApp et choisissez la livraison ou le retrait en magasin.
            </p>
            ...
            <div className="mt-7 grid grid-cols-3 gap-2 border-t border-white/10 pt-5 sm:gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xl font-extrabold text-sun sm:text-2xl">10+</p>
                <p className="text-xs font-bold text-zinc-300">Catégories</p>
              </div>
              <div className="min-w-0">
                <div className="mb-2 grid h-9 w-9 place-items-center rounded-full bg-white/10 text-sun ring-1 ring-white/10">
                  <Clock size={18} strokeWidth={2.5} />
                </div>
                <p className="text-xl font-extrabold text-sun sm:text-2xl">8h-22h</p>
